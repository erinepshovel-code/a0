...
Guardian Tensor — N=29 prime-node microkernel ring.
  - Ragged circle counts per seed: circleCount[i] in [1..12], grows/shrinks with reward
  - AES-256-GCM encryption key management (key derivation, not actual file I/O here)
  - X25519 key exchange + Ed25519 signing stubs
  - Blueprint hash distributed across all 29 nodes
  - Gate control: each node has an open/restricted gate based on coherence threshold
"""
...
class GuardianTensor:
    ...
    # ── circle growth / shrink ────────────────────────────────────────────────
    def apply_reward(self, reward: float):
        """Grow or shrink circle counts based on reward signal."""
        ...
        for i in range(N):
            ...
            self.circle_count[i] = int(np.clip(self.circle_count[i] + delta, MIN_CIRCLES, MAX_CIRCLES))
    ...
    def pcta_circle_audit(self) -> list[dict]:
        ...
        for i in range(N):
            ...
            ring_val = float(self.tensor[i, :, :, :6].mean())
            gate = bool(self.gate_open[i])
            results.append({
                "node": i,
                "circles": circles,
                "hub": round(hub_val, 4),
                "ring_mean": round(ring_val, 4),
                "gate": gate,
                "coherence": round(self.node_coherence[i], 4),
            })
        return results
